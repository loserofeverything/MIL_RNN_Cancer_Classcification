...
import re
import time
from utils import conv_tcr_train_MIL, conv_tcr_test_MIL, \
    tcr_dataset, MyLoss, read_tcr_files, conv_tcr_inference
from models import MODEL, convTCRNet, conv1dTCRNet
...
# if os.path.exists("/xiongjun/test/MIL/tcr_saved/dataset/0.9_tcr_[0.7-0.2-0.1]") is False:
#      os.mkdir("/xiongjun/test/MIL/tcr_saved/dataset/0.9_tcr_[0.7-0.2-0.1]")


# if os.path.isfile("/xiongjun/test/MIL/tcr_saved/dataset/0.9_tcr_[0.7-0.2-0.1]/train.pkl"):
...
#     with open(os.path.join("/xiongjun/test/MIL/tcr_saved/dataset/0.9_tcr_[0.7-0.2-0.1]", \
#                        'train.pkl'), 'wb') as f:
#             pickle.dump(train_dataset, f)

# if os.path.isfile("/xiongjun/test/MIL/tcr_saved/dataset/0.9_tcr_[0.7-0.2-0.1]/val.pkl"):
...
# if os.path.isfile("/xiongjun/test/MIL/tcr_saved/dataset/0.9_tcr_[0.7-0.2-0.1]/test.pkl"):
...
with open("/xiongjun/test/MIL/tcr_saved/dataset/split_label_1_6/train.pkl", 'rb') as f:
    train_dataset = pickle.load(f)
with open("/xiongjun/test/MIL/tcr_saved/dataset/split_label_1_6/val.pkl", 'rb') as f:
    val_dataset = pickle.load(f)
...
train_dataset.df['label'] = (train_dataset.df['label'] == 1).astype(int)
val_dataset.df['label'] = (val_dataset.df['label'] == 1).astype(int)
...
hidden_dim = 128
input_dim = max_length * 5
num_class = 2


# model = MODEL(input_dim, hidden_dim, num_class).to(device)
#model = convTCRNet(hidden_dim, num_class).to(device)
model = conv1dTCRNet(hidden_dim, num_class).to(device)
num_epochs = 100
lr = 0.005
batch_size1 = 1024
batch_size2 = 256
save_dir = "/xiongjun/test/MIL/tcr_saved"
file_dir = "top1_fixed_dim_1_6"
file_name = "conv1dMIL"
...
for epoch in range(num_epochs): 
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size1, shuffle=False, num_workers = 4) 
    topk_list = conv_tcr_inference(model, train_dataloader, device, topk, mode = '1d')
    train_dataset.extract_data(topk = topk_list)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size2, shuffle=True)
    train_loss_, train_acc_ = conv_tcr_train_MIL(model, train_dataloader, criterion, device, optimizer, scaler, mode = '1d')
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size1, shuffle=False, num_workers = 2)
    topk_list = conv_tcr_inference(model, val_dataloader, device, topk, mode ='1d')
    val_dataset.extract_data(topk = topk_list)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size2, shuffle=False)
    val_loss_, val_acc_ = conv_tcr_test_MIL(model, val_dataloader, criterion, device, \
                    picname='val_epoch{}'.format(epoch), mode = '1d',\
                        save_dir = '/xiongjun/test/MIL/tcr_saved/figs/top1_conv1d_figs_1_6')
...
# test_dataset.extract_data()
# test_dataloader = DataLoader(test_dataset, batch_size=batch_size1, shuffle=False) 
# topk_list = conv_tcr_inference(model, test_dataloader, device, topk)
# test_dataset.extract_data(topk = topk_list)
# test_dataloader = DataLoader(test_dataset, batch_size=batch_size2, shuffle=False)
# # 获取目录下的所有文件名
...
# loadmodel = MODEL(input_dim, hidden_dim, num_class).to(device) 
# loadmodel.load_state_dict(torch.load(os.path.join(save_dir, file_name,file_dir, best_model)))
# test_loss_, test_acc_ = conv_tcr_test_MIL(model, test_dataloader, criterion, device, \
#                     picname=best_model.split('.')[0] + "_test", save_dir = '/xiongjun/test/MIL/tcr_saved/top1_conv_figs')
