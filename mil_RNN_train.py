...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        val_loss_, val_acc_ = test_RNN(epoch, embs, rnn, val_dataloader, criterion, device,\
            save_dir=os.path.join(args.save_figs, args.RNN_dir), picname="val_epoch{}".format(epoch))
        ...
        if val_acc_ > best_acc:
            ...
            best_acc = val_acc_
            best_model = os.path.join(os.path.join(args.save_dir, 'RNN', args.RNN_dir), "{}-{}-{}.pth".format(args.save_name, epoch, best_acc))
    ...
    test_loss_, test_acc_ = test_RNN(114514, embs, new_rnn, test_dataloader, \
            criterion, device, save_dir=os.path.join(args.save_figs, args.RNN_dir),\
                picname=best_model.split('.')[0] + "_test")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--RA_path", type=str, default="/xiongjun/test/MIL/data/1_zz_RA/*/*.csv", help="RA path")
